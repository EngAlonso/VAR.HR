    ...
    <div className="company-details-page min-w-0 max-w-full animate-in space-y-6 overflow-x-clip break-words">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-3">{operationalGroups.map(([label, tables]) => <div className="rounded-lg bg-muted/60 p-3" key={label}><div className="text-sm font-semibold">{text(label, label === "Payroll" ? "إدارة الرواتب" : label === "Attendance" ? "الحضور والانصراف" : label === "People & structure" ? "الموظفون والهيكل التنظيمي" : "قواعد العمل والجداول")}</div><div className="mt-2 grid min-w-0 grid-cols-2 gap-1 text-xs text-muted-foreground">{tables.map((table) => <span className="min-w-0 break-words" key={table}>{table.replace("var_hr_", " ").replaceAll("_", " ")}: {details.tableCounts[table] ?? 0}</span>)}</div></div>)}</div>
        </Card>
      </div>
