...
const seedInstant = (hours: number, minutes: number) => {
  const value = new Date(
    `${seedDate}T${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:00+03:00`,
  );
  return value;
};
...
export async function initializeDemoData(): Promise<void> {
  ...
  await db.insert(attendanceTable).values([
    ...
    },
    {
      ...
      source: "biometric",
      explanation:
        "18 minutes after scheduled start; 10 minute grace period applied.",
    },
    {
      companyId: company.id,
...
async function ensureDemoAccounts(companyId: string): Promise<void> {
  ...
  if (!platform) {
    await db.insert(userAccountsTable).values({
      ...
      passwordHash: hashPassword(
        process.env.VAR_HR_PLATFORM_PASSWORD ?? "PlatformDemo2026!",
      ),
    ...
    logger.warn(
      "Created development Platform Owner account platform.owner; set VAR_HR_PLATFORM_PASSWORD outside development.",
    );
  }
  ...
  if (!owner) {
    await db.insert(userAccountsTable).values({
      ...
      passwordHash: hashPassword(
        process.env.VAR_HR_OWNER_PASSWORD ?? "OwnerDemo2026!",
      ),
  ...
  for (const [index, employee] of employees.entries()) {
    const [identity] = await db
      .select({ id: employeeIdentitiesTable.id })
      .from(employeeIdentitiesTable)
      .where(eq(employeeIdentitiesTable.employeeId, employee.id))
      .limit(1);
    if (identity) continue;
    const biometricEmployeeNumber = String(20 + index);
    const username = `A-${biometricEmployeeNumber}`;
    const [account] = await db
      .insert(userAccountsTable)
      .values({
        username,
        passwordHash: hashPassword(generateNumericPassword()),
